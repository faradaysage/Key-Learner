...
Download the [Unity 3.0.0 Windows installer](https://github.com/faradaysage/Key-Learner/releases/download/unity-v3.0.0-preview.1/KeyLearner-3.0.0-win-x64-setup.exe) and [SHA-256 checksum](https://github.com/faradaysage/Key-Learner/releases/download/unity-v3.0.0-preview.1/KeyLearner-3.0.0-win-x64-setup.exe.sha256) from the [GitHub prerelease](https://github.com/faradaysage/Key-Learner/releases/tag/unity-v3.0.0-preview.1). That preview is the previously tested local Unity package. New Unity builds and installers are generated by [windows-installer.yml](.github/workflows/windows-installer.yml) using GameCI on GitHub-hosted runners. Download the Windows player and installer artifacts from a successful [Actions run](https://github.com/faradaysage/Key-Learner/actions/workflows/windows-installer.yml). [CI setup and artifact details](docs/unity-ci.md) explain Personal-license activation, Unity tests, caching, and installer upgrade checks. The package updates the existing per-user installation and retains the parent profile; no separate .NET installation is required. [Verification](docs/unity-verification.md) documents the unresolved native Unity crash risk and target-laptop acceptance limits. See [installer builds and upgrades](docs/installer.md).
