...
Download the [Unity 3.0.0 Windows installer](https://github.com/faradaysage/Key-Learner/releases/download/unity-v3.0.0-preview.1/KeyLearner-3.0.0-win-x64-setup.exe) and [SHA-256 checksum](https://github.com/faradaysage/Key-Learner/releases/download/unity-v3.0.0-preview.1/KeyLearner-3.0.0-win-x64-setup.exe.sha256) from the [GitHub prerelease](https://github.com/faradaysage/Key-Learner/releases/tag/unity-v3.0.0-preview.1). It is the locally built and tested Unity package; [verification](docs/unity-verification.md) documents the unresolved native Unity crash risk and target-laptop acceptance limits. Rebuild it with the licensed Editor using [these commands](UnityPort/README.md#packaging-and-profiles). It updates the existing per-user installation and retains the parent profile. No separate .NET installation is required. The GitHub **KeyLearner-MonoGame-Windows-installer** Actions artifact contains the preserved MonoGame application; it has not been switched to an unconfigured Unity CI license. See [installer builds and upgrades](docs/installer.md).
