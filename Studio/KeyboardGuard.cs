...
public sealed class KeyboardGuard : IDisposable
{
    ...
    public void SetGameActive(bool active){lock(stateGate)focus.SetActive(active && OwnsForeground);}
    ...
    private void Pump()
    {
        ...
        try
        {
            ...
            if(focusHook==0 || desktopHook==0)throw new Win32Exception(Marshal.GetLastWin32Error());
            SetTimer(0,1,1000,0);
            ready.Set();
            while(!disposed && GetMessage(out var message,0,0,0)>0)
            {
                if(message.Id==0x113)
                {
                    if(!OwnsForeground)SetGameActive(false);
