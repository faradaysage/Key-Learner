...
## Protected focus regression checks

The Unity update loop refreshes capture every frame, matching MonoGame. `CaptureFocusHandoff` acknowledges only focus losses for which the frame reset its gameplay and parent state. `KeyboardGuard.SetGameActive` returns that loss counter under the same lock as activation. A native callback arriving around the frame handoff therefore cannot leave capture permanently inactive or hide a pending reset.

Player logs record startup window/fullscreen state, completed capture transitions and counts of physical/synthetic packets, without recording typed key values. The Windows installer starts play with `-screen-fullscreen 1`; Parent Studio explicitly uses `-screen-fullscreen 0`.

Run `scripts/test-unity-protected-focus.ps1 -Executable <player.exe>` on an interactive Windows desktop. It runs actual protected play with a disposable profile and an independent owned-process deadline, checks three native minimize/restore cycles, cursor/accessibility release, normal close while inactive, and unchanged parent files. It does not synthesize physical keystrokes. The two EditMode race cases use the production focus/physical ledger with simulated packets; real keyboard, parent chord and target touchpad acceptance remain separate.
