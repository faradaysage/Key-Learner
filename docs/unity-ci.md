...
3. A hosted Ubuntu job restores the prepared DLL/helper under the existing Unity `Assets` paths and caches `UnityPort/Library` by OS, Editor, packages, and source content. Separate sequential GameCI steps run actual EditMode and PlayMode tests in fresh containers while reusing the Library and image; missing, zero-test, failed, or skipped test results cannot count as success.
