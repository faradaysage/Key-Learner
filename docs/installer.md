...
The Unity Windows package is generated by `.github/workflows/windows-installer.yml`: GameCI builds/tests on GitHub-hosted runners, then Inno Setup packages the Windows installer and verifies a MonoGame-to-Unity upgrade. See [CI setup and downloadable artifacts](unity-ci.md). The same Editor build method and `scripts/build-unity-installer.ps1` also support local builds below.

## Preserved MonoGame packaging

The hosted compatibility job in `.github/workflows/windows-installer.yml` retains the MonoGame tests, native probe and installer validation. Its installer serves as the baseline for the Unity upgrade test; use the separate Unity installer artifact for the migrated application. The package targets Windows 10/11 x64 and bundles the latest .NET 8 runtime patch selected from Microsoft release metadata, fonts, icons, and 70 offline speech clips. Other words use Windows speech unless a parent adds recordings or optional Piper.
...
Download the tested [Unity 3.0.0 Windows prerelease](https://github.com/faradaysage/Key-Learner/releases/tag/unity-v3.0.0-preview.1). Its installer/checksum are uploaded from the locally verified Unity build. New CI runs generate their own Unity player and installer from the checked-out source. Both the preserved model regressions and the shared domain regressions remain, and GameCI runs the Unity Test Framework suites before building. Missing Personal-license secrets fail the Unity job explicitly; see [activation and artifacts](unity-ci.md).
...
The Unity Parent Studio shortcut and optional post-install action start maximized with `-screen-fullscreen 0 --studio --data "%LOCALAPPDATA%\KeyLearner"` (Inno resolves the actual local data directory). MonoGame retains `--preview --studio --data`. The Unity Play shortcuts request maximized startup and borderless fullscreen. This deliberately opens the real parent profile without a keyboard hook. Ordinary `--preview` remains an isolated disposable profile; QA must not reuse the shortcut's real-profile argument. Unity also accepts `--studio` alone for an unprotected real-profile studio.
...
## Installed-game acceptance for the immersion update

Use `scripts/test-unity-installer.ps1 -Installer <downloaded-setup.exe> -VerifyGameplay` on an interactive Windows desktop after the read-only preflight passes. In addition to install/reinstall identity and cleanup checks, this starts the **installed** executable, verifies the branded version splash, captures all thirteen modes with sound, and sends bounded native keys/pointer input to the owned preview for camera cycling, dolphin breach/re-entry, pearl bonus painting and Cannon Hop. Every gameplay run uses an isolated test profile. The report distinguishes requested/completed gameplay acceptance; no Python or TTS model is invoked.

The same procedure is used for the GitHub artifact before calling the update verified. Injected native game input and preview do not establish physical keyboard containment or secure-desktop behavior. Those remain separate device-level checks.
